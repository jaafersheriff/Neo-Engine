...
namespace neo {
	...
	void Renderer::initGPUContext(WindowSurface& window) {
		...
		/* Init GLEW - needs a current context, so it has to happen here rather than on main */
	...
	void Renderer::render(WindowSurface& window, IDemo* demo, util::Profiler& profiler, const ECS& ecs, ResourceManagers& resourceManagers) {
		...
		// Read off the clone, so these are the values ImGui had settled on when the frame was handed
		// over - nothing here can be edited mid-render.
