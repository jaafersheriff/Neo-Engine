...
namespace neo {
	...
	public:
		...
		// The clear value decides the format the driver reinterprets the buffer through, so it has to
		// be a 32-bit type - which is also every type a shader would read out of one.
		template<typename T>
