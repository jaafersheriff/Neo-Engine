...
namespace neo {
	...
	public:
		...
		void bindUniform(HashedString::hash_type, const UniformVariant& uniform) const;
		void bindTexture(const char* name, const Texture& texture) const;
		void bindTexture(HashedString::hash_type, const Texture& texture) const;
	...
	private:
		...
		int32_t _getUniform(HashedString::hash_type hash) const;
	};
}