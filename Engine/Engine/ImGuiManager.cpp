...
#define HEXTOIM(x) x >= 0x1000000 ? ImVec4( ((x >> 24) & 0xFF) /255.f, ((x >> 16) & 0xFF)/255.f, ((x >> 8) & 0xFF)/255.f, ((x) & 0xFF)/255.f): ImVec4( ((x >> 16) & 0xFF) /255.f, ((x >> 8) & 0xFF)/255.f, ((x) & 0xFF)/255.f, 1.f) 
	...
	void ImGuiManager::updateViewport() {
		...
		// Called immediately after Begin("Viewport") with nothing drawn yet, so the cursor is still at
		// the content origin - which makes these exactly the window-local pair they replace, minus the
		// obsolete GetWindowContentRegion* calls.
