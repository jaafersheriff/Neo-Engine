...
void main() {
	...
	uint idx = atomicAdd(fragmentCounter, lock == -1 ? 1 : 0);

	if (idx < outputBufferSize) {
		// First come first serve, only one fragment written out per voxel
		if (lock == -1) {
			fragments[idx].worldPosition = fragPos;
			fragments[idx].albedo = vec4(albedo, 1.0);
			fragments[idx].normal = normalize(fragNor);
			color = vec3(1);
		}
		else {
			// Maybe this new fragment is more important than what was already stored
			float oldLuma = luma(fragments[idx].albedo.rgb);
			float newLuma = luma(albedo);
			if (newLuma > oldLuma) {
				// fragments[idx].worldPosition = fragPos;
				// fragments[idx].albedo = vec4(albedo, 1.0);
				// fragments[idx].normal = normalize(fragNor);
